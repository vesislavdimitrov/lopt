...
tasks=$(curl -u "root:${TEST_PASSWD}" -s "${LOCAL_URL}/tasks")
...
single_task=$(curl -u "root:${TEST_PASSWD}" -s "${LOCAL_URL}/tasks/${task_id}")
...
updated_task=$(curl -u "root:${TEST_PASSWD}" -s "${LOCAL_URL}/tasks/${task_id}")
