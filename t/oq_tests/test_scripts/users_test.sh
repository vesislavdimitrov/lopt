...
all_users=$(curl -u "${USER}:${TEST_PASSWD}" -s GET "${LOCAL_URL}/users")
...
curl -u "${USER}:${TEST_PASSWD}" -s -X DELETE "${LOCAL_URL}/users/testuser" \
    -H "Content-Type: application/json" \
    -d '{"delete_home": 1}'
...
all_users_after_deletion=$(curl -u "${USER}:${TEST_PASSWD}" -s GET "${LOCAL_URL}/users")
